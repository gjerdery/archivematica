...
FidoPath = "/usr/lib/archivematica/archivematicaCommon/externals/FIDO/fido/fido/fido.py"

def getFidoVersion():
    command = "python %s -v" % (FidoPath) 
...
def getFidoID(itemdirectoryPath):
    command = "python %s %s" % (FidoPath, itemdirectoryPath) 
    exitCode, stdOut, stdErr = executeOrRun("command", command, printing=False)
